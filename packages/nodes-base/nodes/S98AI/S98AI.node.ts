...
export class S98AI extends Node {
	authPropertyName = 'authentication';

	description: INodeTypeDescription = {
		displayName: 'S98AI',
		icon: { light: 'file:s98ai.svg', dark: 'file:s98ai.dark.svg' },
		name: 's98ai',
		subtitle:
			'={{ {execution: "On Execution", phoneCall: "On Phone Call End"}[$parameter["triggerType"]] }}',
		group: ['trigger'],
		version: [1, 1.1, 2, 2.1],
		defaultVersion: 2.1,
		...
		properties: [
				...
				options: [
					{
						name: 'Execution',
						value: 'execution',
						description: 'Execute workflow from agent or call from S98AI application',
					},
					{
						name: 'On Phone Call End',
						value: 'phoneCall',
						description: 'Execute workflow when a phone call ends in S98AI application',
					},
				],
				default: 'execution',
				description: 'On which trigger from S98.ai it will start the exeuction',
			},
			...
			{
				displayName:
					'If you sarmad are sending back a response, add a "Content-Type" response header with the appropriate value to avoid unexpected behavior',
